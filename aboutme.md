---
layout: page
title: About me
subtitle: ML/DL Researcher & Engineer
---

안녕하세요 박선흠입니다. 저의 경력사항은 다음과 같습니다 :

- **이투온** (분석팀, 2020.07 ~ 2021.04)  

1. 침입절도 사고 데이터 분석 프로젝트
    - 진행 기관 : ADT 캡스, SKT
    - 프로젝트 목표 : ADT 침입절도 사고 데이터 기반 제주 전지역 위험지수 예측, 순찰경로 추천
    - 데이터 : ADT 사건사고, 출동 데이터, SKT 유동인구, 매출데이터
    - 언어 : python, sql
    - Model : Boosting 알고리즘 비교분석

2. 뉴스 빅데이터 분석 프로젝트
    - 진행 기관 : 소프트웨어 정책연구소(SPRI)
    - 프로젝트 목표 : 에듀테크 산업 관련 뉴스기사 국내, 국외 동향분석
    - 분석 툴 : Gephi
    - Model : Modularity 기반 클러스터링

- **코코네** (AI Lab, 2021.05 ~ 2021.08)  

1. 유저 이탈예측 모델링
    - 일배치 데이터 수집 및 가공, Airflow기반 워크플로우 작업
    - 유저별 AB테스트 진행 및 보고

2. 실시간 비속어 탐지 모델 개발
    - 게임 내 비속어 사용시 블라인드 처리를 위한 탐지 모델 개발(RCNN)

- **천재교과서** (AI센터 개발운영팀, 2021.09 ~ )  

1. 유저 학습패턴 분석 및 클러스터링
    - 유저 활동 지표 정의
    - 분석 결과 기반 학습코칭 제안

2. 전사 데이터 분석 니즈 대응
    - 데이터마트 구축 : 온프레미스 DB 마이그레이션(AWS), 분석용 테이블 생성(AWS-EMR)
    - 분석 대시보드 생성(AWS-Athena)


제가 진행한 AI 프로젝트들을 참조해주세요.
- **Amenity Detection 프로젝트** - CenterNet을 이용한 Custom Dataset에 대한 Object Detection - 방 이미지 내에 존재하는 편의용품(Amenity) 검출 ([링크](https://sunheum.github.io/2022-08-09-Amenity-Detection/))
- **전이학습 프로젝트** - Pytorch를 이용한 전이학습 모델 생성 ([링크](https://sunheum.github.io/2022-09-25-Transfer-Learning-Classification/))
